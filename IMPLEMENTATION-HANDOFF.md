...
- The plasma resolver prelude `YT-SUB:A9BB..AB3D` now uses an injected native
  ...
  cuts retain their exact prefix. A second injected owner covers
  `AB3D..AD17`: same-sector origin/route-array mutation, route-child calls,
  first-versus-later hop loss, the zero-hop/sub-one-energy footer gate, exact
  hop row and half-second wait, non-black-hole impact handoff, and black-hole
  origin/RNG/destination/blank/attention/blank/reroute order. Same-sector
  completion now performs its terminal loss pass; exhausted impacts do not.
...
- The connected `AE00..B099` deployed-fighter continuation now has one
  injected plasma-specific native transaction. It preserves the retained
  binary attacker label from the launch prelude, performs the optional owner
  GET before the `YOU` override, emits the bold encounter before selector
  two, and sends nonpositive incoming energy directly to `B099` without a
  fabricated zero-destruction row. Its DOUBLE damage loop, continuous RNG
  tape, negative-energy zero seam, direct row, strict-greater-than-nine news,
...
- The shared `B26E..B713` cached-player/planet dispatcher now has one native
  ...
  the next connected frontier at that checkpoint.
- The `B2B1..B500` player-impact prefix and `B68E..B6C8` survivor branch now
  have one injected native owner. It performs distinct entry, presentation,
  and survivor player reads; retains the binary launch attacker; keeps
  foreground five active through two news-before-bold-row pairs; consumes
  one continuous fighter-then-shield RNG tape; preserves negative energy;
  and overlays only fresh shields and `CSNG` fighters before the survivor
  PUT. Fixtures pin both damage-loop back-edges, both strict clamp arms,
  survivor and killed partitions, fresh-record/unrelated-byte preservation,
  exact binary rows, foreground lifetime, and every injected callback
  failure prefix. The connected frontier now begins at the `B500`
  killed-player continuation at that checkpoint.
- The `B500..B6D9` killed-player continuation now has one injected native
  owner. It performs a fresh victim read, distinct self/binary-victim rows,
  mines/scanner-only victim PUT, blink-one destruction and optional warning,
  fresh sector mine addition and PUT, and the exact self versus
  death-selector-three-salvage child partition. Its terminal gate warns and
  deploys any nonzero mine total but reloads `B099` only for positive energy
  plus positive saved mines, including positive sub-one energy; negative
  mines instead join the retained-counter/footer gate. Fixtures pin both
  identities, positive/zero/negative mines, child order, raw unrelated-byte
...
- The shared YT-INIT operator-input root `009C`, its sole return and both
  ...
  adapter remain open.
- Both qualified scoreboard-selection transfers at `YT-INIT:0A81/0A83` are
  candidate. Empty input copies `YTSCORE.ASC`; nonempty input bypasses that
...
- The thirteen port-record and planet-initialization controller transfers at
  `1B13..1FC7` are candidate. The complete recovered-RNG fixture pins both
  three-draw inner loops, the Earth/generated-name split and helper join,
  1,000 ascending port records and their output pairs, then all 2,000
  template-derived planet records. The raw 432,235-byte image and terminal
  RNG state expose loop, draw, name, serialization and bound drift. Raw
  FIELD/cursor/loop
  cells, allocation/provider failures, and staged physical-write prefixes
  remain open.
