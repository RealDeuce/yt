...
static const struct yt_clock utility_fixed_clock_source = {
	.read = utility_fixed_clock,
};
...
test_yt_init_pre_input_presentation(void)
{
	...
	const struct yt_clock clock_source = {
		.read = utility_counted_fixed_clock,
		.context = &clock,
	};
	struct yt_random random;
...
test_yt_init_presented_world(void)
{
	...
	ok = yt_init_present_confirmation_prefix(&presenter, &error)
	    ...
	    "YTSCORE.ASC", &clock_source, &random,
	    &presenter, &error);
...
test_yt_init_random_binding(void)
{
	...
	(void)remove("YTDATA.DAT");
	yt_error_clear(&error);
	ok = !yt_initialize_bind_yt(&database, &error)
...
static bool
initialize_unprepared_yt(const char *scoreboard, const struct yt_clock *clock,
...
test_yt_clock_boundaries(void)
{
	...
	const struct yt_clock clock = {
		.read = utility_initializer_clock_sequence,
		.context = &sequence,
	};
	struct yt_random random;
	struct yt_initializer_preparation preparation;
...
initialize_direct(struct yt_error *error)
{
	...
	yt_random_init(&random);
	return initialize_unprepared_yt("YTSCORE.ASC", NULL, &random, error);
}
...
test_immediate_death_cleanup(struct yt_error *error)
{
	...
	valid = sector_cache[2] == 0 && cloak_cache[2] == 0.0f
	    ...
	    && victim.ground_forces == 0.0f && victim.team == 0
...
test_genesis_rmt_producer_consumer(struct yt_error *error)
{
	...
	static const uint8_t handoff_file[] = "DORINFO1.DEF\r\n\x1a";
	...
	if (!result)
		return false;
	result = read_file("RMTINIT.TMP", &file, &file_length)
	    && file_length == sizeof(handoff_file) - 1U
	    && memcmp(file, handoff_file, sizeof(handoff_file) - 1U) == 0;
	free(file);
	if (!result || !run_redirected(YT_RMT_INIT_EXE, "rmt.in", "rmt.out")
	    || !yt_text_read("rmt.out", &output, error))
		return false;
	result = output.length == sizeof(expected_output) - 1U
