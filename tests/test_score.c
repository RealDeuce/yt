...
enum computer_port_earth_cut {
	COMPUTER_PORT_EARTH_FAIL_FIRST_GET,
	COMPUTER_PORT_EARTH_FAIL_OWNER_GET,
	COMPUTER_PORT_EARTH_FAIL_CURRENT_GET,
	COMPUTER_PORT_EARTH_FAIL_CURRENT_AFTER_OWNER,
	COMPUTER_PORT_EARTH_FAIL_AFTER_CURRENT,
	COMPUTER_PORT_EARTH_SUCCEED,
};

struct computer_port_earth_tape {
	enum computer_port_earth_cut cut;
	struct yt_record port;
	struct yt_record owner;
	struct yt_record current;
	size_t calls;
};
    ...
    struct yt_computer_port_earth_state *state, struct yt_error *error)
{
	...
	++tape->calls;
	if (tape->cut == COMPUTER_PORT_EARTH_FAIL_FIRST_GET)
		goto failed;
	state->field_kind = YT_COMPUTER_PORT_EARTH_FIELD_PORT;
	state->field_record = 2056U;
	state->field = tape->port;
	state->field_valid = true;
	if (tape->cut == COMPUTER_PORT_EARTH_FAIL_OWNER_GET)
		goto failed;
	if (tape->cut == COMPUTER_PORT_EARTH_FAIL_CURRENT_AFTER_OWNER) {
	...
	state->field_kind = YT_COMPUTER_PORT_EARTH_FIELD_PLAYER;
	state->field_record = 2U;
	state->field = tape->current;
	if (tape->cut == COMPUTER_PORT_EARTH_FAIL_AFTER_CURRENT)
		goto failed;
	return true;
...
failed:
	if (error != NULL) {
		...
		snprintf(error->operation, sizeof(error->operation), "%s",
		    "computer port Earth child cut");
	}
	return false;
}

static void
computer_port_earth_fixture(struct computer_port_earth_tape *tape,
    ...
    enum computer_port_earth_cut cut)
{
	...
	memset(state, 0, sizeof(*state));
	tape->cut = cut;
	memset(tape->port.bytes, 0x50, sizeof(tape->port.bytes));
	memset(tape->owner.bytes, 0x4f, sizeof(tape->owner.bytes));
	memset(tape->current.bytes, 0x43, sizeof(tape->current.bytes));
	state->field_kind = YT_COMPUTER_PORT_EARTH_FIELD_SECTOR;
...
check_computer_port_earth_transaction(void)
{
	...
	for (cut = COMPUTER_PORT_EARTH_FAIL_FIRST_GET;
	    cut < COMPUTER_PORT_EARTH_SUCCEED; ++cut) {
		computer_port_earth_fixture(&tape, &state, cut);
		yt_error_clear(&error);
		if (yt_computer_port_earth_run(&state,
		    computer_port_earth_child, &tape, &error)
		    || error.status != YT_IO_ERROR || tape.calls != 1U
		    || state.report_calls != 1U || state.report_returned
		    || state.complete)
			return false;
		if (cut == COMPUTER_PORT_EARTH_FAIL_FIRST_GET) {
	...
	computer_port_earth_fixture(&tape, &state,
	    COMPUTER_PORT_EARTH_SUCCEED);
	return yt_computer_port_earth_run(&state, computer_port_earth_child,
	    &tape, NULL) && tape.calls == 1U && state.report_calls == 1U
...
enum computer_scoreboard_event {
	...
	COMPUTER_SCOREBOARD_SELECTOR_PROMPT,
	COMPUTER_SCOREBOARD_EDIT,
	COMPUTER_SCOREBOARD_RESET,
	COMPUTER_SCOREBOARD_TRAILING_BLANK,
	COMPUTER_SCOREBOARD_UPDATED_HEADING,
	COMPUTER_SCOREBOARD_GENERATE,
	COMPUTER_SCOREBOARD_POST_GENERATOR_BLANK,
	COMPUTER_SCOREBOARD_VIEW,
};

struct computer_scoreboard_tape {
	enum computer_scoreboard_event events[12];
	size_t calls;
	size_t fail_at;
	const char *response;
	bool available;
	...
	uint8_t rows[5][96];
	size_t row_lengths[5];
	enum yt_computer_scoreboard_output_kind kinds[5];
	size_t row_count;
	uint8_t pager_raw[4];
...
static bool
computer_scoreboard_step(struct computer_scoreboard_tape *tape,
    enum computer_scoreboard_event event, struct yt_error *error)
{
	size_t call = tape->calls++;
...
static void
computer_scoreboard_clear_test(void *context)
{
	struct computer_scoreboard_tape *tape = context;

	(void)computer_scoreboard_step(tape, COMPUTER_SCOREBOARD_CLEAR, NULL);
}

static bool
computer_scoreboard_present_test(void *context, const uint8_t *text,
    size_t length, enum yt_computer_scoreboard_output_kind kind,
    struct yt_error *error)
{
	struct computer_scoreboard_tape *tape = context;
	static const enum computer_scoreboard_event event[] = {
		COMPUTER_SCOREBOARD_LEADING_BLANK,
		COMPUTER_SCOREBOARD_SELECTOR_PROMPT,
		COMPUTER_SCOREBOARD_TRAILING_BLANK,
		COMPUTER_SCOREBOARD_UPDATED_HEADING,
	...
	if ((size_t)kind >= YT_ARRAY_LEN(event)
	    || row >= YT_ARRAY_LEN(tape->rows)
	    || length > sizeof(tape->rows[0])
	    || !computer_scoreboard_step(tape, event[kind], error))
		return false;
	if (length != 0U)
		memcpy(tape->rows[row], text, length);
	tape->row_lengths[row] = length;
...
static bool
computer_scoreboard_edit_test(void *context, char *response,
    ...
    struct yt_error *error)
{
	...
	if (length == NULL || available == NULL || response_length >= capacity
	    || !computer_scoreboard_step(tape, COMPUTER_SCOREBOARD_EDIT,
	    error))
		return false;
	memcpy(response, tape->response, response_length + 1U);
	*length = tape->report_capacity_length ? capacity : response_length;
	*available = tape->available;
	return true;
}

static void
computer_scoreboard_reset_test(void *context, const uint8_t raw[4])
{
	struct computer_scoreboard_tape *tape = context;

	(void)computer_scoreboard_step(tape, COMPUTER_SCOREBOARD_RESET, NULL);
	memcpy(tape->pager_raw, raw, sizeof(tape->pager_raw));
}

static bool
computer_scoreboard_generate_test(void *context, struct yt_error *error)
{
	return computer_scoreboard_step(context, COMPUTER_SCOREBOARD_GENERATE,
	    error);
}

static bool
computer_scoreboard_view_test(void *context, const char *pathname,
    struct yt_error *error)
{
	struct computer_scoreboard_tape *tape = context;

	tape->viewed_path = pathname;
	return computer_scoreboard_step(tape, COMPUTER_SCOREBOARD_VIEW, error);
}

static const struct yt_computer_scoreboard_ops
computer_scoreboard_test_ops = {
	computer_scoreboard_clear_test,
	computer_scoreboard_present_test,
	computer_scoreboard_edit_test,
	computer_scoreboard_reset_test,
	computer_scoreboard_generate_test,
	computer_scoreboard_view_test,
};

static void
computer_scoreboard_fixture(struct computer_scoreboard_tape *tape,
    struct yt_computer_scoreboard_state *state, const char *response)
{
	memset(tape, 0, sizeof(*tape));
	memset(state, 0xa5, sizeof(*state));
	tape->fail_at = SIZE_MAX;
	tape->response = response;
	tape->available = true;
	state->pathname = "YTSCORE.ASC";
}

static bool
check_computer_scoreboard_transaction(void)
{
	static const uint8_t prompt[] =
	    "Enter 'O' to see OLD scoreboard or press [ENTER] for UPDATED one. -=>";
	static const uint8_t heading[] = "P l a y e r  R a n k i n g s";
	static const enum computer_scoreboard_event old_events[] = {
		COMPUTER_SCOREBOARD_CLEAR,
		COMPUTER_SCOREBOARD_LEADING_BLANK,
		COMPUTER_SCOREBOARD_SELECTOR_PROMPT,
		COMPUTER_SCOREBOARD_EDIT,
		COMPUTER_SCOREBOARD_RESET,
	...
	static const enum computer_scoreboard_event updated_events[] = {
		COMPUTER_SCOREBOARD_CLEAR,
		COMPUTER_SCOREBOARD_LEADING_BLANK,
		COMPUTER_SCOREBOARD_SELECTOR_PROMPT,
		COMPUTER_SCOREBOARD_EDIT,
	...
	static const char *const updated_responses[] = {
		"", "OLD", "O ", "OO", "E",
	};
	static const size_t updated_failable[] = {
		1U, 2U, 3U, 5U, 6U, 7U, 8U, 9U,
	};
	struct computer_scoreboard_tape tape;
	struct yt_computer_scoreboard_state state;
	struct yt_error error;
	size_t index;

	computer_scoreboard_fixture(&tape, &state, "o");
	if (!yt_computer_scoreboard_run(&state,
	    &computer_scoreboard_test_ops, &tape, NULL)
	    || !state.complete || !state.pager_key_cleared
	    || !state.selector_presented || !state.input_available
	    || !state.pager_reset || state.updated || state.generator_called
	    || state.generator_complete || !state.viewer_called
	    || state.raw_response_length != 1U
	    || memcmp(state.raw_response, "o\0", 2U) != 0
	    || state.response_length != 1U
	    || memcmp(state.response, "O\0", 2U) != 0
	    || memcmp(state.pager_line_count_raw, "\0\0\4\0", 4U) != 0
	    || memcmp(tape.pager_raw, "\0\0\4\0", 4U) != 0
	    || tape.calls != YT_ARRAY_LEN(old_events)
	    || memcmp(tape.events, old_events, sizeof(old_events)) != 0
	    || tape.row_count != 3U || tape.row_lengths[0] != 0U
	    || tape.kinds[0] != YT_COMPUTER_SCOREBOARD_LEADING_BLANK
	    || tape.row_lengths[1] != sizeof(prompt) - 1U
	    || tape.kinds[1] != YT_COMPUTER_SCOREBOARD_SELECTOR_PROMPT
	    || memcmp(tape.rows[1], prompt, sizeof(prompt) - 1U) != 0
	    || tape.row_lengths[2] != 0U
	    || tape.kinds[2] != YT_COMPUTER_SCOREBOARD_TRAILING_BLANK
	    || strcmp(tape.viewed_path, "YTSCORE.ASC") != 0)
		return false;

	for (index = 0U; index < YT_ARRAY_LEN(updated_responses); ++index) {
		computer_scoreboard_fixture(&tape, &state,
		    updated_responses[index]);
		if (!yt_computer_scoreboard_run(&state,
		    &computer_scoreboard_test_ops, &tape, NULL)
		    || !state.complete || !state.updated
		    || !state.generator_called || !state.generator_complete
		    || !state.viewer_called
		    || tape.calls != YT_ARRAY_LEN(updated_events)
		    || memcmp(tape.events, updated_events,
		    sizeof(updated_events)) != 0 || tape.row_count != 5U
		    || tape.row_lengths[3] != sizeof(heading) - 1U
		    || tape.kinds[3]
		    != YT_COMPUTER_SCOREBOARD_UPDATED_HEADING
		    || memcmp(tape.rows[3], heading,
		    sizeof(heading) - 1U) != 0
		    || tape.row_lengths[4] != 0U
		    || tape.kinds[4]
	...
	for (index = 0U; index < YT_ARRAY_LEN(updated_failable); ++index) {
		computer_scoreboard_fixture(&tape, &state, "");
		tape.fail_at = updated_failable[index];
		yt_error_clear(&error);
		if (yt_computer_scoreboard_run(&state,
		    &computer_scoreboard_test_ops, &tape, &error)
		    || error.status != YT_IO_ERROR || state.complete
		    || tape.calls != updated_failable[index] + 1U
		    || memcmp(tape.events, updated_events,
		    tape.calls * sizeof(updated_events[0])) != 0)
			return false;
	}
	...
	if (yt_computer_scoreboard_run(&state,
	    &computer_scoreboard_test_ops, &tape, NULL)
	    || !state.pager_key_cleared || !state.selector_presented
	    || state.input_available || state.pager_reset || state.complete
	    || tape.calls != 4U)
		return false;
	computer_scoreboard_fixture(&tape, &state, "");
	...
	if (yt_computer_scoreboard_run(&state,
	    ...
	    || error.status != YT_RANGE
	    || strcmp(error.operation,
	    "scoreboard selector response capacity") != 0
	    || tape.calls != 4U)
		return false;
	computer_scoreboard_fixture(&tape, &state, "O");
	return !yt_computer_scoreboard_run(NULL,
	    &computer_scoreboard_test_ops, &tape, NULL)
...
main(void)
{
	...
	if (!check_computer_port_earth_transaction())
		return fail("computer-port Earth transaction differs");
