    ...
    struct yt_error *error)
{
	...
	for (sector = 1; sector <= 7; ++sector) {
		...
		if (!yt_game_read_sector(game, sector, &record, error))
			return false;
		record.mines = 0.0f;
		if (!yt_game_write_sector(game, sector, &record, error))
			return false;
	}
	return true;
}
    ...
    struct yt_error *error)
{
	...
	while (cursor < bulletin.length && bulletin.data[cursor] != 0x1a) {
		...
		while (cursor < bulletin.length
		    && bulletin.data[cursor] != '\r'
		    && bulletin.data[cursor] != '\n'
		    && bulletin.data[cursor] != 0x1a)
			++cursor;
		if (!line_output(context, bulletin.data + start, cursor - start,
		    error))
			goto done;
		if (cursor < bulletin.length && bulletin.data[cursor] == '\r')
			++cursor;
		if (cursor < bulletin.length && bulletin.data[cursor] == '\n')
			++cursor;
	}
	result = true;
    ...
    struct yt_error *error)
{
	return yt_maintenance_xannor_planet_arrival(&state->game,
	    &location[group], &size[group], sector, maintenance_stdout_line,
	    NULL, error);
}
    ...
    struct yt_error *error)
{
	...
	success = xannor_route_arrivals_impl(&state, group_number,
	    target_sector, (float)target_sector, location, size, result, error);
		...
		if (!split_result.skip_group) {
			do {
				...
				if (!xannor_candidate_target(state,
				    ...
				    || !maintenance_stdout_line(NULL,
				    group_output.rows[0].data,
				    group_output.rows[0].length, error))
					return false;
