    ...
    bool *enter_sector, struct yt_error *error)
{
	...
	for (;;) {
		...
		if (strcmp(upper, "N") == 0) {
			if (!yt_session_planet_rename(session, logical_planet, NULL, error))
				return false;
			continue;
		}
		if (strcmp(upper, "S") == 0) {
		...
		if (strcmp(upper, "D") == 0) {
		if (!yt_session_planet_inventory(session, logical_planet, error))
				return false;
			continue;
		}
		if (strcmp(upper, "?") == 0) {
		...
		case 5: case 6: case 7: case 8: case 9: case 10:
			...
			break;
		case 11:
			if (!yt_session_planet_take_one(session, logical_planet, 9, error))
				return false;
			break;
		case 12:
			if (enter_sector != NULL)
		...
		case 15:
			...
			break;
		case 16:
			if (!yt_session_planet_productivity(session, logical_planet, error))
				return false;
			break;
		default:
			break;
    ...
    struct yt_error *error)
{
	...
	if (permission_denied) {
		...
		if (!yt_session_planet_assault(session, physical, commitment, &defeated,
		    error))
			return false;
		if (defeated) {
			if (enter_sector != NULL)
