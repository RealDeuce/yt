    ...
    size_t owner_name_length, struct yt_error *error)
{
	...
	if (!purchase_present_sold(session, error)
	    || !read_port_physical(session, physical_port, &port, error))
		return false;
	if (old_owner != 0.0f) {
		int seller_record;

		length = 0U;
		if (!port_append(row, sizeof(row), &length, transfer_prefix,
		    sizeof(transfer_prefix) - 1U)
		    || !port_append(row, sizeof(row), &length, owner_name,
		    owner_name_length)
		    || !port_append(row, sizeof(row), &length, transfer_suffix,
		...
		length = 0U;
		if (!port_append(message, sizeof(message), &length,
		    cached_trader, cached_trader_length)
		    || !port_append(message, sizeof(message), &length, radio_one,
		    sizeof(radio_one) - 1U)
		    || !port_append(message, sizeof(message), &length, old_name,
		    old_name_length)
		    || !port_append(message, sizeof(message), &length, radio_two,
		    sizeof(radio_two) - 1U)
		    || !port_append(message, sizeof(message), &length,
		    sector_text, strlen(sector_text))
		    || !port_append(message, sizeof(message), &length, radio_three,
		    sizeof(radio_three) - 1U)
		    || !port_append(message, sizeof(message), &length,
		    price_text, strlen(price_text))
		    || !port_append(message, sizeof(message), &length, radio_four,
	...
	if (relative_port > 1.0f
		...
		return false;
	if (!read_port_physical(session, physical_port, &port, error)
	...
	if (!yt_database_write(&session->door->game.database,
		...
		return false;
	length = 0U;
	if (!port_append(row, sizeof(row), &length, success_prefix,
	    sizeof(success_prefix) - 1U)
	    || !port_append(row, sizeof(row), &length, first_name,
	    first_name_length)
	    || !port_append(row, sizeof(row), &length, success_suffix,
    ...
    struct yt_error *error)
{
	...
	if (old_owner == (float)session_record(session)) {
		length = 0U;
		if (!port_append(row, sizeof(row), &length, already_prefix,
		    sizeof(already_prefix) - 1U)
		    || !port_append(row, sizeof(row), &length, first_name,
		    first_name_length)
		    || !port_append(row, sizeof(row), &length, "!", 1U))
			return treasury_error(error,
	...
	if (old_owner != 0.0f) {
		...
		if (!session_port_owner_row_capture(session, &display_port,
			...
			return false;
		length = 0U;
		if (!port_append(row, sizeof(row), &length, offer_prefix,
		    sizeof(offer_prefix) - 1U)
		    || !port_append(row, sizeof(row), &length, owner_name,
		    owner_name_length)
		    || !port_append(row, sizeof(row), &length, offer_suffix,
