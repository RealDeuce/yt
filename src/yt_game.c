...
static float
salvage_single_mul(float left, float right)
{
	volatile float result = left * right;
...
bool
yt_salvage_run(struct yt_salvage_state *state,
    ...
    struct yt_error *error)
{
	...
	if (state == NULL || ops == NULL || ops->read_victim == NULL
	    || ops->read_killer == NULL || ops->write_killer == NULL
	    || ops->random == NULL || ops->cargo_draw == NULL
	    || ops->wait == NULL || ops->present == NULL || ops->news == NULL
	    || (state->current_name == NULL && state->current_name_length != 0U))
		return false;
	state->admitted = false;
	state->emitted = false;
	state->complete = false;
	state->requested_holds = 0.0f;
	memset(state->opening_draws, 0, sizeof(state->opening_draws));
	memset(state->awards, 0, sizeof(state->awards));
	memset(&state->cargo, 0, sizeof(state->cargo));
	if (!ops->read_victim(context, state->victim_record, &state->victim,
	    error))
		return false;
	if (state->killer_record < 2.0f
	    || state->killer_record > state->last_player_record) {
		state->complete = true;
		return true;
	}
	state->admitted = true;
	if (!yt_player_stored_name(&state->victim, victim_name,
	    &victim_name_length, error)
	    || !ops->present(context, NULL, 0U, false, error)
	    || !ops->present(context, title, sizeof(title) - 1U, true, error)
	    || !yt_salvage_header_row(state->current_name,
	    state->current_name_length, victim_name, victim_name_length,
	    row, sizeof(row), &row_length)
	    || !ops->news(context, row, row_length, error)
	    || !ops->present(context, NULL, 0U, false, error))
		return false;
	for (index = 0U; index < YT_ARRAY_LEN(state->opening_draws); ++index) {
		float stock;

		if (!ops->random(context, &state->opening_draws[index], error))
			return false;
		switch (index) {
		case 0U: stock = state->victim.holds; break;
		case 1U: stock = state->victim.credits; break;
		case 2U: stock = state->victim.missiles; break;
		case 3U: stock = state->victim.plasma; break;
		case 4U: stock = state->victim.ground_forces; break;
		default: stock = state->victim.mines; break;
		}
		state->awards[index] = floorf(salvage_single_mul(
		    state->opening_draws[index], stock));
	}
	if (!ops->wait(context, 1.0f, error)
	    || !ops->read_killer(context, state->killer_record,
	    &state->killer, error))
		return false;
	simple_fields[0] = &state->killer.credits;
	simple_fields[1] = &state->killer.missiles;
	simple_fields[2] = &state->killer.plasma;
	simple_fields[3] = &state->killer.ground_forces;
	simple_fields[4] = &state->killer.mines;
	for (index = 1U; index < YT_ARRAY_LEN(state->awards); ++index) {
		if (state->awards[index] == 0.0f)
			continue;
		if (!ops->wait(context, 0.5f, error))
			return false;
		state->emitted = true;
		if (!yt_salvage_simple_row(
		    (enum yt_salvage_simple_kind)(index - 1U),
		    state->awards[index], row, sizeof(row), &row_length)
		    || !ops->news(context, row, row_length, error)
		    || !ops->present(context, row, row_length, false, error))
			return false;
		*simple_fields[index - 1U] = salvage_single_add(
		    *simple_fields[index - 1U], state->awards[index]);
	}
	if (!ops->write_killer(context, state->killer_record,
	    &state->killer, error))
		return false;
	state->requested_holds = state->awards[0];
	if (salvage_single_add(state->killer.holds, state->requested_holds)
	    > state->maximum_holds)
		state->requested_holds = salvage_single_sub(state->maximum_holds,
		    state->killer.holds);
	if (state->requested_holds > 0.0f) {
		state->emitted = true;
		state->cargo.requested = state->requested_holds;
		if (!ops->read_victim(context, state->victim_record,
		    &state->victim, error))
			return false;
		state->cargo.stock[0] = state->victim.ore;
		state->cargo.stock[1] = state->victim.organics;
		state->cargo.stock[2] = state->victim.equipment;
		state->cargo.remaining = state->victim.holds;
		if (!yt_salvage_cargo_sample(&state->cargo, ops->cargo_draw,
		    context, error)
		    || !ops->read_killer(context, state->killer_record,
		    &state->killer, error))
			return false;
		for (index = 0U; index < YT_ARRAY_LEN(state->cargo.awards);
		    ++index)
			state->killer.holds = salvage_single_add(
			    state->killer.holds, state->cargo.awards[index]);
		state->killer.ore = salvage_single_add(state->killer.ore,
		    state->cargo.awards[0]);
		state->killer.organics = salvage_single_add(
		    state->killer.organics, state->cargo.awards[1]);
		state->killer.equipment = salvage_single_add(
		    state->killer.equipment, state->cargo.awards[2]);
		if (!ops->write_killer(context, state->killer_record,
		    &state->killer, error)
		    || !ops->wait(context, 0.5f, error))
			return false;
		for (index = 0U; index < YT_ARRAY_LEN(cargo_order); ++index) {
			...
			if ((award == 3U && state->cargo.awards[award] <= 0.0f)
			    || (award != 3U
			    && state->cargo.awards[award] == 0.0f))
				continue;
			if (!ops->wait(context, 0.5f, error)
			    || !yt_salvage_cargo_row(cargo_kind[index],
			    state->cargo.awards[award], row, sizeof(row),
			    &row_length)
			    || !ops->news(context, row, row_length, error)
			    || !ops->present(context, row, row_length, false, error))
				return false;
	...
	if (!state->emitted) {
		if (!ops->wait(context, 0.5f, error)
		    || !ops->news(context, nothing, sizeof(nothing) - 1U, error)
		    || !ops->present(context, nothing, sizeof(nothing) - 1U,
		    false, error))
			return false;
	}
    ...
    struct yt_error *error)
{
	...
	return yt_current_date_serial_observed(
	    game->config.record.bytes + YT_F45, &game->today,
	    &game->adjusted_year, NULL, NULL, error);
}
    ...
    struct yt_error *error)
{
	...
	static const uint8_t selector_three[4] = {
		0x00U, 0x00U, 0x40U, 0x82U,
	};
	static const uint8_t duration_five[4] = {
	...
	if (!ops->sound(context, selector_three, error)
	    ...
	    || !ops->wait(context, duration_five, error))
		return false;
