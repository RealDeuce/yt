...
bool
session_earth_lottery(struct yt_session *session, const struct yt_port *cached_earth,
    struct yt_error *error)
{
	char ticket[80];
	...
	}
	{
		...
		if (qb_str_single(limit, sizeof(limit),
			...
			return false;
		if (snprintf(row, sizeof(row), "You may play%s times daily.",
		    limit) < 0)
			return false;
	...
	for (;;) {
		...
		if (!session_present_timed_paged_row(session,
			...
			return false;
		if (!session_read_command(session, ticket, sizeof(ticket)))
			return false;
		if (strlen(ticket) != 6U) {
			if (!session_present_text(session,
	...
	}
	{
		...
		if (qb_str_single(match_text, sizeof(match_text),
		    (float)matches) < 0)
			return false;
		if (qb_str_double(award_text, sizeof(award_text),
		    (double)award) < 0)
			return false;
		if (!session_present_text(session,
