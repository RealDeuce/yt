...
| `yt_out_present_result()` | The documented logical event tape remains the compatibility model. Its active endpoint is replayed through public OpenDoors combined-output, color, cursor, bell, and clear calls; there is no private framebuffer or direct `ODScrn` path. |
