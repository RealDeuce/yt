...
| `od_disp(buffer, length, TRUE)` in `yt_out_plain*` | Used only for plain literal/editor bytes established not to contain terminal-emulator commands; the names make that provenance requirement explicit, and presentation replay uses separate exact remote and local sinks |
| remote events in `yt_out_present_result()` | Each recovered COM1 `PRINT` statement traverses the shared write-behind device reducer, preserving the per-byte BRUN index, column, stale byte, pending count, and selected-control state across statements. After that exact state transition succeeds, the accepted bytes use the existing counted remote-only `od_disp()` calls; they are not split into extra calls that would introduce additional OpenDoors kernel checkpoints |
| `yt_out_opening_file("YTOPEN.ANS")` | The exact remote semicolon row followed by literal LF/newline traverses the same COM1 reducer before its existing counted calls; the trusted NUL-free local lines and final reset use `od_disp_emu(..., FALSE)`, so the Win32 local display interprets their ANSI commands without retransmitting them |
